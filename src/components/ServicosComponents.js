...
import icon0 from '../assets/Icon/icon_0.png';
...
const servicos = [
  ...
  { 
    ...
    icone: icon3, 
    descricao: 'Asseguramos que sistemas operacionais, aplicativos e ferramentas específicas sejam instalados corretamente, evitando incompatibilidades e falhas. O serviço inclui a ativação de licenças, personalização de configurações e atualização de softwares, garantindo desempenho otimizado e seguro para os usuários.' 
  },
  { 
    titulo: 'Segurança de Redes', 
    icone: icon4, 
    descricao: 'Implementamos soluções essenciais para proteger dados contra ameaças cibernéticas, como invasões, roubo de informações e ataques de malware. Utilizamos firewalls, criptografia, VPNs e monitoramento constante para evitar acessos não autorizados, garantindo a integridade e confidencialidade das informações, prevenindo riscos e vulnerabilidades.' 
  },
  { 
    ...
    <div className="servicoscomponent-wrapper">
      <div className="servicoscomponent-grid">
        {servicos.map((servico, index) => (
          <div className={`servicoscomponent-card-wrapper ${index % 2 === 0 ? 'with-line' : ''}`} key={index}>
            <div className="servicoscomponent-card">
              <img src={servico.icone} alt={servico.titulo} className="servicoscomponent-icon" />
